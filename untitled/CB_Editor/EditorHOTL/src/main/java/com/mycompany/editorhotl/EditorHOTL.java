...
//public class Main {
    public static void main(String[] args) throws IOException {
       ...
       // printlvlPointerDetails(1,rom);
        int result;
        //result=getlvlPointerDetails(2,rom);
        //levelDetail lvl=getlvlPointerDetails(2,rom);
        
        for (int i = 1; i < 89; i++) {//89
//            getPitRAMPointers(rom,i);       //Prints the pointer in the RAM dumps, that are used for the floor collisions.
//    result= getPitArrayLength(i);    
//    byte[] pa=new byte[result];
//    pa=returnPitArray(i);
//            for (int j = 0; j < pa.length;++j) {
 //               System.out.println((pa[j]&0xFF));
 //truncateRAMdump(i);
  //          }
//returnPitArray
   ...
   // testPrint(rom,i);
        ...
        //getPitRAMPointers(rom,1);       //Prints the pointer in the RAM dumps, that are used for the floor collisions.
...
try {
    ...
    }   //This is just a template, does nothing.
    ...
    }   //Check if the csv is there. I think this in not connected to anything.
    ...
    } //An existing csv gets opened. Since this part is not implemented, it does nothing.
    ...
    }   //Prints the character selection text on the console.   Maybe this is not needed for now, whatever.
...
fos.close();
    ...
    }   //As the name suggests, it prints the character statistics.
...
//_LABEL_D52:
        ...
        System.out.println("_RAM_DE60_DOOR_POINTER: "+String.format("0x%02X", _RAM_DE60_));
    ...
    }   //Prints level load and other details, that the game uses of course, when loading a level.
...
return dt;
    ...
    /*
    These below are for extracting information on deadly pits on a level. Some RAM pointers are necessary, otherwise you can just comb through the level loading code, but this is easier.
    I commented on things pretty extensively. There's a pit class, but it's not needed at the moment, as it's easier to deal with them this way.
    I'll do some other helping code later to extract them into csv files, and to reimport it to an unaltered rom later on.
    The pit data is packed in the middle of level descriptors and door data, so adding them is currently a bit difficult. Changing and optimizing them however is absolutely possible.
    */
    public static void printPitRAMPointer(int roomnr){  //will return data about the deathpits in the game.
    int pitPos=0x175B;  //This is where the game stores the pit vals.
    int noPit=0x1761;   //Val for levels where there is nothing to jump over.
    int rampitpointer=0x1e5e;   //Pit pointer into rom in the RAM dump files.
    String ramdump_folder="/media/work/devver/disas/HOTL/anotherexam/editor/room_ram/HOTL_mod_ah";
    int fileNr=roomnr;      //This is maybe not needed, but whatever.
    int pointerResult;
    String ext=".ram";
    String wholefile=ramdump_folder + String.valueOf(fileNr)+ext;   //Soo, this would produce a full file link and changeable based on the room nr we need.
    System.out.println("RAM file will be opened: "+wholefile);  
    byte[] ramdump=readRom(wholefile);          //Read the file into the array, so we can get the pointer.
    pointerResult=((((ramdump[rampitpointer+1])&0xFF) <<8 )| (ramdump[rampitpointer]& 0xFF));   //Connect the first and the second byte together, since this is a pointer.  This will be the final ROM address in an unaltered file, or where the data is in the same place.
    //System.out.println(Integer.toBinaryString(pointerResult )); //Print this to the console in a binary format, just to see that it's right.
        if (pointerResult==noPit) {
            System.out.println("ROM death pit pointer at address: 0x"+(Integer.toHexString(pointerResult))+" LEVEL HAS NO PITS!");     //Print this, so we know the hex value is right.
        }
        else{
        System.out.println("ROM death pit pointer at address: 0x"+(Integer.toHexString(pointerResult)));     //Print this, so we know the hex value is right.
        }
    
    pointerResult=Integer.reverseBytes(pointerResult);          //Change endianness of the number, as we have to do this in order to later on put this stuff back into the ROM if needed.
    //System.out.println(Integer.toHexString(pointerResult));     //Check if the reversed number is looking good or not. It is good, but later on when data is put back, we have to do some work.
    /*
    The thing should work as the following:
        Get the ROM, and the address where these details are, just see above.   -done.
        Get an array back, until we get a zero back.
        Zero terminates the given room's pit list.
        First byte is the starting column.
        Second is the ending column.
        Third byte is either a zero, which is then the end of the given room's pit list OR
        the next pit start and end coordinates are coming.
    
        Never seen any pit data starting with zero, so that's not a problem here.
        There's a bunch of RAM dumps at a folder on my machine, which should be the same on anyone else's, or change the value to something else in the code.
    */
    }   //As the name suggests, this prints the ROM pointers that are stored in RAM, when the room is loaded.
    public static int getPitRamPointer(int roomnr){
    ...
    }   //This will return an int with the correct ROM location where each pit is in the ROM.
    public static int getPitArrayLength(int roomnr){
        /*Reads the ROM, and given the room nr, it should give back the byte length of the trap assigned for it.
        The length should include all bytes with the terminating zero.
        
        */
    byte[] rom=readRom("/media/work/devver/disas/HOTL/anotherexam/editor/Lance.sms");
    int result=0;
    int PitPointer=getPitRamPointer(roomnr);    //We get the ROM address for a given pit.
    byte romval;    //This will hold the number of vals retrieved from ROM.
    while(true)
    {
    romval=rom[PitPointer]; //Get a val from rom.
        if (romval!=0) {
            result++;   //Count up if the val is not zero.
            PitPointer++;
        }
        else{       //Else just break out of the loop. We've finished counting. There's definetly an easier way for this, don't care.
        break;
        }
    }
            

    return result;  //Then we get the result. The zero needs a plus one, if needed, i'll figure it out.
}   //Returns the length of the would be array of pits for a given room. Don't forget to add the trailing zero later on.
    public static byte[] returnPitArray(int roomnr){
        byte[] rom=readRom("/media/work/devver/disas/HOTL/anotherexam/editor/Lance.sms");
        int PitPointer=getPitRamPointer(roomnr);    //We get the ROM address for a given pit.
        int arrlen=getPitArrayLength(roomnr);   //Get a nice array length. A list may be better but whatever again.
        byte[] pitarray=new byte[arrlen+1];     //The ending zero is also important.
        int arrpnt=0;
    byte romval;    //This will hold the number of vals retrieved from ROM.
    while(true)
    {
    romval=rom[PitPointer]; //Get a val from rom.
        if (romval!=0) {
            pitarray[arrpnt]=romval;
            arrpnt++;
            PitPointer++;
        }
        else{       //The ending zero needs to be stored, for completeness' sake.
            pitarray[arrpnt]=romval;    
        break;  //And then escape.
        }
    }
    
        return pitarray;
    }   //Returns a variable length array with the pit coordinates. Unsigned integer of course.
    public static void testPrint(byte[]rom,int roomNr){
            List<String> roomDetails=new ArrayList<>(); //What is printed will be returned in a List, so later on, a csv could handle this.
        /*
         * I think for the time, the command line part is also okay, and the data that we will put out would be useable for a GUI stuff.
         */
    ...
    }   //A shorter version to print out level details, but it might be not needed at all.
    public static void truncateRAMdump(int roomNr) {
    byte[]result =new byte[0x1000]; //One whole level is four kilobytes. This is what we have to reduce later on. I wonder how much better another compression tool can do.
       String ramdump_folder="/media/work/devver/disas/HOTL/anotherexam/editor/room_ram/HOTL_mod_ah";
    int fileNr=roomNr;      //This is maybe not needed, but whatever.
    int pointerResult;
    String ext=".ram";
    ...
    System.out.println("RAM file will be opened: "+wholefile);  
    byte[] ramdump=readRom(wholefile);          //Read the file into the array, so we can get the pointer.
        for (int i = 0; i < result.length; i++) {       //We put one array's content into another, but only the first 4k. The rest is not needed at the moment.
            result[i]=ramdump[i];
        }
        writeTruncRAMdump(result,wholefile);
    //return result;
    }       //Reads a RAM dump file, and gets the first four kilobytes. This is where the whole level's data is. The rest of this file is discarded. The truncated file is then written to disk, to the same place the as the ramdumps.
    public static void writeTruncRAMdump(byte[] input,String filename){
            String extra="trunc";
            File file = new File(filename+extra);   //We need a new fileanem for the truncated RAM file.
    ...
    }   //A separate method to write these truncated bytes into a file.
}
